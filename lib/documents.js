...
function Paragraph(children, properties) {
    ...
    return {
        ...
        listBreak: properties.listBreak || false,
...
function MathElement(children, properties) {
    properties = properties || {};
    return {
        type: types.math,
        name: properties.name || "math",
        children: children,
...
// `Buffer` exists only in Node: browser bundlers no longer polyfill it. Handing
// back a `Uint8Array` instead would be worse than failing, since its
// `toString("base64")` silently returns comma-separated digits rather than
// base64, so point the caller at the readers that do work in the browser.
function toNodeBuffer(arrayBuffer) {
    if (typeof Buffer === "undefined") {
        throw new Error(
            "Buffer is not defined: read() without an encoding and readAsBuffer() require Node. " +
                "Use readAsArrayBuffer() or readAsBase64String() in the browser."
        );
    }
    ...
    return {
        ...
        read: function(encoding) {
            if (encoding) {
                ...
                return options.readImage().then(toNodeBuffer);
            }
        },
...
function OleObject(options) {
    return {
        type: types.oleObject,
        progId: options.progId,
        displayName: options.displayName,
        altText: options.altText,
        readIcon: options.iconReader
    };
}

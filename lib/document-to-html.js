...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        ...
        var hasBorders = hasParagraphBorders(element.borders);
        var content;
        var nodes = htmlPathForParagraph(element, messages).wrap(function() {
            content = convertElements(element.children, messages, options);
            if (ignoreEmptyParagraphs && !hasBorders) {
                return content;
            } else {
        ...
        if (hasBorders) {
            nodes = addParagraphBorders(nodes, element.borders);
        }
        if (preserveAlignment) {
            nodes = addParagraphAlignment(nodes, element.alignment, content || []);
        }
    ...
    function hasParagraphBorders(borders) {
        return borders && _.any(["top", "bottom", "left", "right"], function(side) {
            return !!borders[side];
        });
    }

    function addParagraphAlignment(nodes, alignment, content) {
        if (alignment === "both" || alignment === "distribute" || alignment === "thaiDistribute") {
            alignment = "justify";
        }
        if (["left", "center", "right", "justify", "start", "end"].indexOf(alignment) === -1) {
            return nodes;
        }

        var applied = false;
        return nodes.map(align);

        function align(node) {
            if (node.type !== "element" || content.indexOf(node) !== -1) {
                return node;
            }
            // Style the paragraph's innermost block wrapper so shared list and
            // section wrappers can still merge without aligning their siblings.
            var children = node.children.map(align);
            var tag = node.tag;
            if (!applied && /^(p|h[1-6]|li|div|blockquote|pre|section|article|aside|header|footer)$/.test(tag.tagName)) {
                tag = copyTagWithStyle(tag, "text-align: " + alignment);
                applied = true;
            }
            return Html.elementWithTag(tag, children);
        }
    ...
    function addParagraphBorders(nodes, borders) {
        var styles = [];
        ["top", "bottom", "left", "right"].forEach(function(side) {
            var border = borders[side];
            if (border) {
                styles.push("border-" + side + ": " + paragraphBorderCss(border));
                if (border.space != null && (side === "top" || side === "bottom")) {
                    styles.push("padding-" + side + ": " + paragraphBorderSpaceCss(border.space));
                }
            }
        });
        ...
        return nodes.map(function(node) {
            if (node.type === "element") {
                return Html.elementWithTag(copyTagWithStyle(node.tag, style), node.children);
            } else {
                return node;
            }
        });
    ...
    function paragraphBorderCss(border) {
        var size = parseInt(border.size, 10);
        var width = isNaN(size) ? "1px" : Math.max(1, size / 6) + "px";
        var styles = {
            single: "solid",
            double: "double",
            dashed: "dashed",
        ...
        var lineStyle = styles[border.style] || "solid";
        var color = /^[0-9A-Fa-f]{6}$/.test(border.color || "") ? "#" + border.color : "currentColor";
        return width + " " + lineStyle + " " + color;
    }

    function paragraphBorderSpaceCss(space) {
        var points = parseInt(space, 10);
        return isNaN(points) ? "0pt" : Math.max(0, points) + "pt";
    }
    ...
    function convertTable(element, messages, options) {
        var nodes = findHtmlPath(element, defaultTablePath).wrap(function() {
            return convertTableChildren(element, messages, options);
        });
        if (!preserveAlignment || ["left", "center", "right"].indexOf(element.alignment) === -1) {
            return nodes;
        }
        var style = "margin-left: " + (element.alignment === "left" ? "0" : "auto") +
            "; margin-right: " + (element.alignment === "right" ? "0" : "auto");
        return nodes.map(alignTable);

        function alignTable(node) {
            if (node.type !== "element") {
                return node;
            } else if (node.tag.tagName === "table") {
                return Html.elementWithTag(copyTagWithStyle(node.tag, style), node.children);
            } else {
                return Html.elementWithTag(node.tag, node.children.map(alignTable));
            }
        }
    }
    ...
    var elementConverters = {
        ...
        "oleObject": deferredConversion(convertOleObject),
        "math": function(element, messages, conversionOptions) {
            if (options.outputFormat === "markdown") {
                return [Html.text(mathToText(element))];
            }
            var attributes = _.extend({}, element.attributes);
            if (element.name === "math") {
                attributes = _.extend({xmlns: "http://www.w3.org/1998/Math/MathML"}, attributes);
                if (preserveAlignment && ["left", "right", "center"].indexOf(element.alignment) !== -1) {
                    attributes.style = "text-align: " + element.alignment;
                }
            }
            // MathML argument slots and adjacent tokens must not be removed or
            // merged by the HTML simplifier.
            var children = convertElements(element.children, messages, conversionOptions);
            return [Html.freshElement(element.name, attributes, [Html.forceWrite].concat(children))];
        },
        "table": convertTable,
...
function convertOleObject(element, messages) {
    var fallbackLabel = element.displayName || element.altText || element.progId || "Embedded Object";
    return promises.when(resolveOleObject(element, fallbackLabel)).then(function(resolved) {
        var children = [];
        if (resolved.iconSrc) {
            var iconAttributes = {
                src: resolved.iconSrc,
                alt: resolved.label || element.altText
            };
            if (resolved.iconWidth && resolved.iconHeight) {
                iconAttributes.width = String(resolved.iconWidth);
                iconAttributes.height = String(resolved.iconHeight);
            } else {
                iconAttributes.width = "32";
                iconAttributes.height = "32";
            }
            children.push(Html.freshElement("img", iconAttributes));
            if (!resolved.iconIncludesLabel) {
                children.push(Html.text(" "));
            }
        }
        if (!resolved.iconIncludesLabel) {
            children.push(Html.text(resolved.label));
        }
        return [Html.freshElement("span", {"class": "embedded-object"}, children)];
    });
}

function resolveOleObject(element, fallbackLabel) {
    if (!element.readIcon) {
        return fallbackOleObjectPreview(element, fallbackLabel);
    }
    return promises.attempt(function() {
        return element.readIcon();
    }).then(function(arrayBuffer) {
        var label = fallbackLabel;
        var extractedText = ole.extractText(arrayBuffer);
        if (extractedText && /\S/.test(extractedText) && !ole.looksLikeMojibake(extractedText)) {
            label = extractedText;
        }
        var preview = ole.createPreviewIcon(arrayBuffer, label);
        return preview ? {
            label: label,
            iconSrc: preview.src,
            iconWidth: preview.width,
            iconHeight: preview.height,
            iconIncludesLabel: true
        } : fallbackOleObjectPreview(element, label);
    }).caught(function() {
        return fallbackOleObjectPreview(element, fallbackLabel);
    });
}

function fallbackOleObjectPreview(element, label) {
    var preview = ole.fallbackIcon(element, label);
    return {
        label: label,
        iconSrc: preview ? preview.src : null,
        iconWidth: preview ? preview.width : null,
        iconHeight: preview ? preview.height : null,
        iconIncludesLabel: !!preview
    };
}

...
var headersFootersReader = require("./header-footer-reader");
...
function read(docxFile, input, options) {
    ...
    return promises.props({
        ...
        partPaths: findPartPaths(docxFile, {
            includeHeadersAndFooters: options.includeHeadersAndFooters
        }),
        docxFile: docxFile,
        files: files
        ...
        return {
            ...
            headers: options.includeHeadersAndFooters
                ? readXmlFilesWithBody(result.partPaths.headers, result, function(bodyReader, xml) {
                    return headersFootersReader.createHeaderReader(bodyReader)(xml);
                })
                : new Result([]),
            footers: options.includeHeadersAndFooters
                ? readXmlFilesWithBody(result.partPaths.footers, result, function(bodyReader, xml) {
                    return headersFootersReader.createFooterReader(bodyReader)(xml);
                })
                : new Result([])
        };
    }).also(function(result) {
...
function findPartPaths(docxFile, options) {
    options = options || {};
        ...
        return xmlFileReader({
            ...
            function findPartRelatedToMainDocument(name) {
                return findPartPath({
                    ...
                    relationshipType: officeDocumentRelationshipType(name),
                    basePath: zipfile.splitPath(mainDocumentPath).dirname,
                    fallbackPath: "word/" + name + ".xml"
                });
            }

            var basePath = zipfile.splitPath(mainDocumentPath).dirname;
            var result = {
            ...
            if (options.includeHeadersAndFooters) {
                return readXmlFromZipFile(docxFile, mainDocumentPath).then(function(mainDocument) {
                    result.headers = findHeaderFooterPartPaths({
                        docxFile: docxFile,
                        relationships: documentRelationships,
                        basePath: basePath,
                    ...
                    result.footers = findHeaderFooterPartPaths({
                        docxFile: docxFile,
                        relationships: documentRelationships,
                        basePath: basePath,
                        mainDocument: mainDocument,
...
function findPartPath(options) {
    ...
    var validTargets = findValidTargets(options);
...
function findValidTargets(options) {
    ...
    return normalisedTargets.filter(function(target) {
        return docxFile.exists(target);
    });
}

function findHeaderFooterPartPaths(options) {
    return findReferencedRelationshipIds(options.mainDocument, "w:" + options.name + "Reference")
        .map(function(relationshipId) {
            var target = options.relationships.findTargetByRelationshipId(relationshipId);
            return target ? stripPrefix(zipfile.joinPath(options.basePath, target), "/") : null;
        })
        .filter(function(target) {
            return target && options.docxFile.exists(target);
        });
}

function findReferencedRelationshipIds(element, name) {
    var relationshipIds = [];
    findElementsByName(element, name).forEach(function(element) {
        var relationshipId = element.attributes["r:id"];
        if (relationshipId && relationshipIds.indexOf(relationshipId) === -1) {
...
function findElementsByName(element, name) {
    if (!element || !element.children) {
        return [];
    }

    var elements = element.name === name ? [element] : [];
...
function officeDocumentRelationshipType(name) {
...
function readXmlFilesWithBody(filenames, options, func) {
    return promises.all(filenames.map(function(filename) {
        return readXmlFileWithBody(filename, options, func);
    })).then(Result.combine);
}

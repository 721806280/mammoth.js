...
var mathReader = require("./math-reader");
...
function BodyReader(options) {
        ...
        return readParagraphStyle(element).map(function(style) {
            return {
                ...
                alignment: element.firstOrEmpty("w:jc").attributes["w:val"] ||
                    styles.findParagraphAlignmentById(style.styleId),
                numbering: readNumberingProperties(style.styleId, element.firstOrEmpty("w:numPr"), numbering),
                listBreak: isNumberingDisabled(element.firstOrEmpty("w:numPr")),
    ...
    function readParagraphBorders(element) {
        ...
        return {
            top: readParagraphBorder(element.first("w:top")),
            bottom: readParagraphBorder(element.first("w:bottom")),
            left: readParagraphBorder(element.first("w:left")),
            right: readParagraphBorder(element.first("w:right"))
        };
    }

    function readParagraphBorder(element) {
        if (!element || element.attributes["w:val"] === "nil" ||
            element.attributes["w:val"] === "none") {
            return null;
        }
        var attributes = element.attributes;
        return {
            style: attributes["w:val"] || "single",
            size: attributes["w:sz"] || null,
            space: attributes["w:space"] || null,
            color: attributes["w:color"] || null
        };
    }
    ...
    function readFldSimple(element) {
        // A simple field (w:fldSimple) carries its instruction in the
        // w:instr attribute and its cached result in the child elements.
        // Treat it like a complex field: parse the instruction, read the
        // result content while the field is on the stack (so hyperlinks are
        // recognised), then pop it.
        var instrText = element.attributes["w:instr"] || "";
        var complexField = parseInstrText(instrText, xml.emptyElement);
        complexFieldStack.push(complexField);
        currentInstrText = [];
        var result = readChildElements(element);
    ...
    function readMath(element) {
        var result = mathReader.read(element);
        return elementResultWithMessages(result.value, result.messages);
    }

    var oleDisplayNamesByProgId = {
        "Excel.Sheet.12": "Excel Worksheet",
        "Excel.Sheet.8": "Excel Worksheet",
        "Excel.Chart.8": "Excel Chart",
        "Word.Document.12": "Word Document",
        "Word.Document.8": "Word Document",
        "PowerPoint.Show.12": "PowerPoint Presentation",
        "PowerPoint.Show.8": "PowerPoint Presentation",
    ...
    var xmlElementReaders = {
        ...
        "w:object": function(element) {
            return readOleWrappedObject(element);
        },
    ...
    function findImageRelationshipIdInObject(element) {
        // The EMF icon for an OLE object lives in a v:imagedata nested inside
        // a wrapping v:shape. getElementsByTagName only searches direct
        // children, so step through v:shape first.
        var shapes = element.getElementsByTagName("v:shape");
        for (var i = 0; i < shapes.length; i++) {
            var imagedata = shapes[i].getElementsByTagName("v:imagedata")[0];
            if (imagedata && imagedata.attributes["r:id"]) {
                return imagedata.attributes["r:id"];
            }
        }
        return null;
    }

    function readOleWrappedObject(element) {
        var oleElement = element.getElementsByTagName("o:OLEObject")[0];
        if (!oleElement) {
            return readChildElements(element);
        }
        if (!oleElement.attributes["r:id"]) {
            return emptyResultWithMessages([warning("An OLE object without a relationship ID was ignored")]);
        }
        return elementResult(buildOleObject(oleElement, findImageRelationshipIdInObject(element)));
    }

    function buildOleObject(oleElement, iconRelationshipId) {
        var relationshipId = oleElement.attributes["r:id"];
        if (!relationshipId) {
            return null;
        }
        var progId = oleElement.attributes.ProgID;
        var fallbackName = oleDisplayNamesByProgId[progId] || progId || "Embedded Object";

        var iconReader = null;
        if (iconRelationshipId) {
            var iconFile = findEmbeddedImageFile(iconRelationshipId);
            iconReader = iconFile.read;
        }

        return documents.OleObject({
            progId: progId,
    ...
    function readOleObject(element) {
        // Stand-alone o:OLEObject (not wrapped in w:object): no EMF icon is
        // available, so fall back to the ProgID-derived display name.
        if (!element.attributes["r:id"]) {
            return emptyResultWithMessages([warning("An OLE object without a relationship ID was ignored")]);
        }
        var oleObject = buildOleObject(element, null);
        return oleObject
            ? elementResult(oleObject)
            : emptyResult();
    }
...
function isNumberingDisabled(element) {
    return element.firstOrEmpty("w:numId").attributes["w:val"] === "0";
}

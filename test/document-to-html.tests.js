...
test('paragraph alignment is preserved and invalid CSS values are ignored', function() {
    var document = documents.Document(["center", "right", "left", "both", "distribute", "start", "end", "center; color: red"].map(function(alignment) {
        return documents.Paragraph([runOfText("Text")], {alignment: alignment});
    }));
    return new DocumentConverter({preserveAlignment: true}).convertToHtml(document).then(function(result) {
        assert.equal(result.value,
            ...
            '<p style="text-align: justify">Text</p>' +
            '<p style="text-align: start">Text</p>' +
            '<p style="text-align: end">Text</p><p>Text</p>');
    });
});

test('alignment is applied to headings and preserves paragraph borders', function() {
    var document = documents.Paragraph([runOfText("Title")], {
        alignment: "center",
        borders: {bottom: {style: "single"}}
    });
    var converter = new DocumentConverter({preserveAlignment: true, styleMap: [{
        from: documentMatchers.paragraph(),
        to: htmlPaths.topLevelElement("h1", {"class": "title"})
    }]});
    return converter.convertToHtml(document).then(function(result) {
        assert.equal(result.value, '<h1 class="title" style="border-bottom: 1px solid currentColor; text-align: center">Title</h1>');
    });
});

test('paragraph alignment does not split shared wrappers or leak to other paragraphs', function() {
    var document = documents.Document([
        documents.Paragraph([runOfText("Center")], {alignment: "center"}),
        documents.Paragraph([runOfText("Normal")])
    ]);
    var converter = new DocumentConverter({preserveAlignment: true, styleMap: [{
        from: documentMatchers.paragraph(),
        to: htmlPaths.elements([
            htmlPaths.element("div", {"class": "section"}),
            htmlPaths.element("p", {}, {fresh: true}),
            htmlPaths.element("span")
        ])
    }]});
    return converter.convertToHtml(document).then(function(result) {
        assert.equal(result.value, '<div class="section"><p style="text-align: center"><span>Center</span></p><p><span>Normal</span></p></div>');
    });
});

test('individual list items can be aligned without splitting the list', function() {
    var document = documents.Document([
        documents.Paragraph([runOfText("Center")], {alignment: "center"}),
        documents.Paragraph([runOfText("Right")], {alignment: "right"}),
        documents.Paragraph([runOfText("Normal")])
    ]);
    var converter = new DocumentConverter({preserveAlignment: true, styleMap: [{
        from: documentMatchers.paragraph(),
        to: htmlPaths.elements([htmlPaths.element("ul"), htmlPaths.element("li", {}, {fresh: true})])
    }]});
    return converter.convertToHtml(document).then(function(result) {
        assert.equal(result.value, '<ul><li style="text-align: center">Center</li><li style="text-align: right">Right</li><li>Normal</li></ul>');
    });
});

test('table alignment does not change cell text alignment or nested table alignment', function() {
    var innerTable = documents.Table([documents.TableRow([documents.TableCell([paragraphOfText("Nested")])])], {alignment: "left"});
    var document = documents.Table([documents.TableRow([documents.TableCell([
        documents.Paragraph([runOfText("Heading")], {alignment: "center"}), innerTable
    ])])], {alignment: "center"});
    return new DocumentConverter({preserveAlignment: true}).convertToHtml(document).then(function(result) {
        assert.equal(result.value,
            '<table style="margin-left: auto; margin-right: auto"><tr><td>' +
            '<p style="text-align: center">Heading</p>' +
            '<table style="margin-left: 0; margin-right: auto"><tr><td><p>Nested</p></td></tr></table>' +
            '</td></tr></table>');
    });
});

test('alignment is omitted from the default HTML output', function() {
    var document = documents.Table([documents.TableRow([documents.TableCell([
        documents.Paragraph([runOfText("Text")], {alignment: "center"})
    ])])], {alignment: "center"});
    return new DocumentConverter().convertToHtml(document).then(function(result) {
        assert.equal(result.value, '<table><tr><td><p>Text</p></td></tr></table>');
    });
});
